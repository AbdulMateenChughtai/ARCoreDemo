...
class PlaneRenderer(render: SampleRender?) {
    ...
    private lateinit var indexBufferObject: IndexBuffer
    private lateinit var vertexBufferObject: VertexBuffer
    ...
    private val planeAngleUvMatrix =
        FloatArray(4) // 2x2 rotation matrix applied to uv coords.
    private val normalVector = FloatArray(3)
    private val planeIndexMap: MutableMap<Plane, Int> =
        HashMap()
    ...
    fun drawPlanes(
        ...
        val sortedPlanes: MutableList<SortablePlane> =
            ArrayList()
        ...
        Collections.sort(
            sortedPlanes
        ) { a, b -> java.lang.Float.compare(b.distance, a.distance) }
        ...
        for (sortedPlane in sortedPlanes) {
            ...
            planeAngleUvMatrix[0] =
                (+Math.cos(angleRadians.toDouble())).toFloat() * uScale
            planeAngleUvMatrix[1] =
                (-Math.sin(angleRadians.toDouble())).toFloat() * vScale
            planeAngleUvMatrix[2] =
                (+Math.sin(angleRadians.toDouble())).toFloat() * uScale
            planeAngleUvMatrix[3] =
                (+Math.cos(angleRadians.toDouble())).toFloat() * vScale
    ...
    private class SortablePlane internal constructor(
        val distance: Float,
        val plane: Plane
    )
     ...
     */
    init {
        render?.let { nonNullRender ->
            val texture: Texture = Texture.createFromAsset(
                nonNullRender,
                TEXTURE_NAME,
            ...
            vertexBufferObject =
                VertexBuffer(render, COORDS_PER_VERTEX,  /*entries=*/null)
